...
void Start_Scene::Init()
{
	...
	Vec2 vResolution = Core::GetInst()->GetResolution();
	float turtleSpawnX = vResolution.x / 2;
	for (int i = 0; i < 5; i++)
	{
		Turtle* pTurtle = new Turtle;
		pTurtle->SetScale(Vec2(100.f, 100.f));
		pTurtle->SetPos((Vec2(turtleSpawnX + 64.f * i, vResolution.y)));
		AddObject(pTurtle, OBJECT_GROUP::PLAYER);
	}
